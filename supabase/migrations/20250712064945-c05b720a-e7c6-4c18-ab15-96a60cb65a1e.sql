...
-- Create secure role management policies (only if they don't exist)
DO $$
BEGIN
  -- Remove existing policy if it exists, then recreate it securely
  ...
  -- Create secure management policy
...
DO $$
BEGIN
  DROP POLICY IF EXISTS "Authenticated users can create quotes securely" ON public.quotes;
  CREATE POLICY "Authenticated users can create quotes securely" 
  ON public.quotes 
  FOR INSERT 
  WITH CHECK (auth.uid() IS NOT NULL AND auth.uid() = created_by);

  DROP POLICY IF EXISTS "Admins can manage all quotes securely" ON public.quotes;
  CREATE POLICY "Admins can manage all quotes securely" 
  ON public.quotes 
  FOR ALL 
...
-- Phase 3: Add foreign key constraint for CMS articles author validation (if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'fk_cms_articles_author'
  ) THEN
...
-- Phase 6: Secure citation policies
DO $$
BEGIN
  ...
  CREATE POLICY "Authenticated users can add citations securely" 
  ON public.cited_by 
  FOR INSERT 
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Super admins can manage all articles securely" ON public.cms_articles;
  
  CREATE POLICY "Super admins can manage all articles securely" 
